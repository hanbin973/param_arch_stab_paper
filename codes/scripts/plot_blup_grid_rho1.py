"""
plot_blup_grid_rho1.py — Combined summary plot for BLUP validation R^2 across
all rho values when the stabilizing model is fixed at rho=1.
...
def collect_metric(results_root: str, rho_true: float, vs_scale: float,
                   sigma2_a: float, metric: str) -> pd.Series:
...
def main(output: str):
    results_root = "results/blup_rho1_fixed"
    out_dir = os.path.dirname(output) or "."
    os.makedirs(out_dir, exist_ok=True)
    vs_scales, sigma2_as, alpha_grid, rhos = load_config()
    ws_trues = [v / 2.0 for v in vs_scales]

    method_specs = [("r2_stabilized", "Evolutionary", "#4c9be8")]
    alpha_palette = alpha_colors(len(alpha_grid))
    method_specs.extend([
        (f"r2_alpha_{alpha_tag(alpha_model)}", fr"$\alpha={alpha_model:g}$", color)
    ...
    fig, axes = plt.subplots(
        len(rhos),
        len(sigma2_as),
        figsize=(2.88 * len(sigma2_as), 2.592 * len(rhos) + 0.648),
        sharex=True,
        sharey="row",
        squeeze=False,
    )
    fig.suptitle(r"Best linear unbiased prediction (BLUP) - Held out, fixed $\rho_{ab}^2=1.0$", fontsize=16, y=0.99)
    fig.legend(
        handles=[
        ...
        ncol=len(method_specs),
        frameon=False,
        fontsize=10,
        handlelength=2.2,
        columnspacing=1.4,
    )
    fig.supxlabel(r"True $W_s$", fontsize=13, y=0.01)

    for row, rho_true in enumerate(rhos):
        for col, sa_true in enumerate(sigma2_as):
            ax = axes[row, col]
            if row == 0:
                ax.set_title(fr"True $\sigma_a^2 = {sa_true}$", fontsize=13)
            ...
            for i, vs_scale in enumerate(vs_scales):
                for method_idx, spec in enumerate(method_specs):
                    series_by_method[method_idx].append(
                        collect_metric(results_root, rho_true, vs_scale, sa_true, spec[0])
                    )
    ...
    print(f"Saved BLUP fixed-rho1 summary plot to {output}", flush=True)


if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python scripts/plot_blup_grid_rho1.py <output>")
        raise SystemExit(1)
    main(sys.argv[1])
