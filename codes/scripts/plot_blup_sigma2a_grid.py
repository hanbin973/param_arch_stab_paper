"""
plot_blup_sigma2a_grid.py — Boxplots of BLUP sigma2_a estimates across methods,
combined over all rho values.

Usage:
    python scripts/plot_blup_sigma2a_grid.py --output plots/blup_sigma2a_estimates.pdf
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot BLUP sigma2_a estimates across methods and parameter settings."
    )
    parser.add_argument("--results-base", default="results/blup")
    parser.add_argument("--output", default="plots/blup_sigma2a_estimates.pdf")
...
def main() -> int:
    ...
    if sample_df.empty:
        print("No BLUP result CSVs found.")
        return 0

    method_specs = detect_method_specs(sample_df, alpha_grid)
    if not method_specs:
        print("No sigma2_a estimate columns found in BLUP results.")
        return 0
    ...
    print(f"Saved sigma2_a estimate plot to {args.output}", flush=True)
    return 0
