"""
plot_blup_grid.py — Combined summary plot for BLUP validation R^2 across all
rho values.
...
def main(output: str):
    ...
    fig.suptitle(r"Best linear unbiased prediction (BLUP) - Held out", fontsize=16, y=0.99)
    fig.legend(
        handles=[
    ...
    print(f"Saved BLUP summary plot to {output}", flush=True)


if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python scripts/plot_blup_grid.py <output>")
        raise SystemExit(1)
    main(sys.argv[1])
