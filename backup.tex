...
We now return to deriving $\E{\beta^2 \mid G}$ where we approximate it by a product of separate loci,
\begin{align} \label{eq:mean_field}
    p(\beta \mid G) = \prod_j p(\beta_j \mid G_j),
\end{align}
and expand $p(\beta_j \mid G_j)\ \propto \ p(G_j \mid \beta_j) \cdot p(\beta_j)$ with the results from the diffusion approximation.
...
\end{align}
where $k_j = 2n \hp_j$ is the number of the minor allele $j$ in the sample and $\pinf(x_j \mid \beta_j)$ is the equilibrium distribution of the population frequency $x_j$.
...
\begin{align} \label{eq:beta_cond_var}
    \E{\beta_j^2 \mid G} = \E{\beta_j^2 \mid \hp_j} = 
    \sigma_{b,j}^2 \cdot 
    \left[
    1 - \rho_{ab,j}^2 \cdot
        \frac{
    ...
    \right]
\end{align}
with effectively three parameters $\sigma_b^2$, $\sigma_a^2 / W_S$, and $\rho_{ab}$.
...
In plain words, $\sigma_b^2$ controls the overall magnitude of $\beta$, $\sigma_a^2/W_S$ determines the strength of stabilizing selection on $z$, and $\rho_{ab}$ measures how strongly $y$ and $z$ are correlated.
...
Secondly, it is always bounded above by $\sigma_b^2$, so does not diverge to infinity regardless of the value of $\hp$.
Nevertheless, it captures the negative correlation between $\hp$ and the magnitude of $\beta$.
Finally, it captures the intuition from earlier studies that the relative ratio of mutational variance $\sigma_{a}^2$ and selection width $W_S$ determines the system [cite, Hayward].
...
\begin{table}[htpb]
    ...
    \begin{tabular}{ll}
        ...
        $x_j, \hp_j$ & Population and sample minor allele frequency of locus $j$ \\
    ...
    \end{tabular}
\end{table}
We consider an observed trait $y$ and an (potentially) unobserved trait $z$ where the latter determines the fitness of an individual.
...
\begin{align} \label{eq:definetti}
    \begin{aligned}
        p(G \mid \alpha, \beta) &=
        \int p(G \mid \alpha, \beta ,P_t)
        \cdot p(P_t \mid \alpha, \beta) dP_t \\
        &=
        \int p(G \mid P_t)
        \cdot p(P_t \mid \alpha, \beta) dP_t \\
        &=
        \int \underbrace{p(G \mid P_t)}_{\text{Sampling}}
...
As long as $\alpha$ is given, evolution of $P_t$ does not depend on $\beta$ anymore.
