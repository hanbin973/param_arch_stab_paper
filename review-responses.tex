...
In this revision, we have clarified both the presentation and scope of the manuscript. 
In particular, we expanded the introduction to better motivate fitness landscapes and stabilizing selection, reorganized the theory to distinguish the general framework from its specific modeling choices, and clarified the roles of $\alpha$ and $\beta$, the pleiotropy model, and the interpretation of the variance parameters. 
We also made the role of the DFE prior more explicit, added an appendix showing that the main functional form extends to a Gamma DFE, and expanded the results and discussion to explain the relationship between the evolutionary model, vanilla $\alpha$-models, and MAF-stratified $\alpha$-models. 
Finally, we added figures and accompanying discussion that visualize the frequency-dependent variance curve and compare it against $\alpha$-model baselines, and we addressed the absence of a real data analysis while explaining the framework's expected empirical applications.
...
This approach is standard with examples including stratified LDSC \citep{Gazal_2017}, GREML-LDMS \citep{Lee_2013,Yang_2015}, and LDAK \citep{Speed2017}.
...
\reply{
We explain in Appendix~\ref{apx:Gamma_dfe} that the formula remains the same under Gamma distributed fitness effect, 
a common choice in DFE modelling \citep{EyreWalker2007}.
The $\chi^2$-distributed case appears as a special instance of this general fact.
The main text was updated accordingly in \revref.
}
...
the suggested derivation likely refers to the use of Wright's formula (equation 21 of the same Bulmer reference).
