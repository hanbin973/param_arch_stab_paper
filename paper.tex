...
\begin{abstract}
    ...
    we study how complex trait should be analyzed in the first place under stabilizing selection.
    ...
    Next, we take into account for pleiotropy and show that functional annotations acts on variance components through trait-fitness genetic correlation.
\end{abstract}
...
a tremendous amount of genetic variants associated to complex traits have been discovered through genome-wise association studies (GWAS) [cite].
...
Some of these methods aims to reveal the overall landscape of how genetic variants contribute to trait genetics, which is referred to as the \emph{genetic architecture}.
...
This magnitude of effect sizes is measured in average squared effect size in which the average is taken over with respect to the \emph{random-effects model}.

In quantitative genetics, linear models involving both fixed and random effects are called \emph{mixed-effects model} and has a long history of its use in quantitative genetics.
...
individuals within appear correlated due to shared ancestry.
...
these Bayesian extensions lack biological interpretability of their priors: What biological and technical randomness do the random-effects represent [cite, Steinsaltz]?
...
Depending on how the trait is determined, it may or may not be suited to survive and spread through the population.
...
Several follow-up studies have appeared and confirmed that stabilizing selection is pivotal force in shaping the genetic architecture of human complex traits [cite].
...
In doing so, we first clarify the source of genetic random-effects as coming from mutations randomly hitting the genome,
...
Then, following recent works in stabilizing selection, we take into account for genetic drift and natural selection to further refine the random effects distribution.
...
With the correctly specified frequency-dependent formulation, 
...
\textcolor{red}{notation table}

We consider an observed trait $y$ and an unobserved trait $z$ where the latter determines the fitness of an individual.
We assume an \emph{additive model} whereby both $y$ and $z$ is weighted sum of genotypes [cite].
\begin{gather}
    \begin{gathered}
        y_i = G_i\beta + \veps \\
        z_i = G_i\alpha + \delta
...
\begin{align} \label{eq:definetti}
    ...
    \end{aligned}
\end{align}
Here, $p(G \mid \alpha, \beta, P_t) = p(G \mid P_t)$ because once the state of the population $P_t$ is given, choosing $G$ is entirely determined by the study's sampling scheme.
...
For the sake of easing computation,
we assume normality
...
In particular, the choice of $\Sigma_\beta = \var{\beta \mid G}$ was assumed making its parameters $h^2$ and $\alpha$ merely a tuning parameter without an explicit biological interpretation, although heuristic arguments exist.
...
A notable pathologic behavior is that $[p_j(1-p_j)]^{-\alpha}$ diverges to infinity as $p_j$ approaches zero, requiring heuristic GRM partitioning over multiple MAF bins [cite, Wainschtein].
...
the final formula will naturally expressed by evolutionary parameters of the system.
...
In a panmictmic diploid population of size $N$,
...
We choose to measure $V_S$ in the units of $2N$,
...
Fortunately, $_1F_1(a;b;z) = 1 + \mathcal{O}(a)$, so we can ignore the factor with $\theta \rightarrow 0$.
...
\begin{align} \label{eq:beta_cond_var}
    ...
    \right]
\end{align}
with effectively three parameters $\sigma_b^2$, $\alpha_a^2 / W_S$, and $\rho$.
Note that $W_S$ and $\alpha_a^2$ cannot be separately identifies because the formula remains invariant with respect to a common scaling factor.
